...
実装パターン（`CredentialsSignin` サブクラスの使用・構造化ログ・クライアントへの安全なエラー返却）は @.claude/rules/app.md の「NextAuthエラーハンドリング」を参照。
...
`.env`・APIキー・秘密鍵・認証情報を含むファイルは、いかなる状況でも読み取り・変更・作成しない（グローバル CLAUDE.md「機密ファイル」と共通ルール）。
