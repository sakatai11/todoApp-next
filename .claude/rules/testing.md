...
PRレビューコメントを修正する際は、以下のプラクティスを徹底してください：

1. **ユニットテストの実行**
   - 修正がモック環境（MSW）で正常に動作することを確認します。

   ```bash
   npm run test:run  # 全ユニットテスト実行
   ```

2. **統合テストの実行**
   - 修正がFirebase Emulatorなどの実環境に近い状態で正常に動作することを確認します。
   ```bash
   npm run docker:test:run  # Firebase Emulator環境で統合テスト
   ```

**理由**: ユニットテストと統合テストの両方を実行することで、モックと実コンポーネントの動作の差異を早期に検出し、修正内容が実際の環境で失敗することを防ぎます。特にAPI通信やデータベース操作に関わる修正では、この両方のテストが重要です。
