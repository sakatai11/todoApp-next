...
1. **各ファイル修正後に完全なテストスイートを実行**
   ```bash
   npm run test:run  # 全ユニットテスト実行
   ```

2. **モックと実コンポーネントの動作の違いを早期検出**
   - MSWモック環境での単体テストと、Firebase Emulator環境での統合テストの両方を実行
   - モックデータと実データの整合性を確認
...
**理由**: 修正内容がモック環境でのみ動作し、実際の環境で失敗することを防ぐため。特にAPI通信やデータベース操作に関わる修正では、早期のテスト実行が重要です。
