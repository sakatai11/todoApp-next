...
- **単体テスト成功率**: CI結果に準拠（全テストパス必須）
- **カバレッジ目標**:
  - **通常**: 100%を目標（Firebase通信・外部API等は統合テストで検証）
  - **現状**: 100%達成済み（413テスト成功）
  - **CI環境**: GitHub Actions（`.github/workflows/`で定義）
  - **カバレッジレポート**: CI実行時に自動生成され、GitHub Actionsのアーティファクトとして保存
