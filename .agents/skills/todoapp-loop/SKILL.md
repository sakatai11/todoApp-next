...
description: |-
  ...
  `/todoapp-loop` で起動し、処理すべきタスクがなくなるまで内部でサイクルを繰り返す。
  1サイクル = 状態読込→triage→優先案件選択→creator委譲→verifierゲート→Draft PR→状態更新→継続判定。
  繰越キューまたは未処理Issueが残っていれば自動でPhase 0へ戻り、すべて空になったら自ら停止する。

  使うべき状況: ユーザーが「ループを回して」「/todoapp-loop」と言った時。
  引数: --dry-run（Phase 2の選択結果を表示して終了。委譲・PR作成・状態更新・ループ継続を行わない）
...
「エージェントに直接プロンプトする人間」を置き換える自律開発ループ。
このスキル自身はコードを書かない・レビューしない。**既存スキルを正しい順序で呼び出し、処理すべきタスクがなくなるまで内部でループし続けるオーケストレーター**である。
...
- 繰越キューに1件以上残っている
- `gh issue list --state open` で処理済みリストに含まれない未処理Issueが存在する
- CI失敗が未解消のまま残っている
...
**継続の場合**: サマリー末尾に `🔄 次サイクルを開始します（繰越N件 / 未処理IssueN件）` と出力し、**そのまま Phase 0 へ戻って次サイクルを開始する**。外部コマンドは不要。

**停止の場合**: サマリー末尾に `✅ 処理すべきタスクがなくなりました。ループを終了します。` と出力し、**スキルを終了する**。
