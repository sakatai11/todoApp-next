...
プロジェクトルートに `.Codex-pr-config.json` がある場合は読み込む。設定ファイルの詳細は [branch-config-example.json](references/branch-config-example.json) を参照。
...
1. **設定ファイルがある場合**: `.Codex-pr-config.json` のブランチパターンマッチング
