...
describe('useUpdateStatusAndCategory', () => {
  ...
  describe('リスト編集 (editList)', () => {
    ...
    it('API呼び出しが失敗した場合、エラー通知が表示されクライアント状態は更新されない', async () => {
      const consoleSpy = vi
        .spyOn(console, 'error')
        .mockImplementation(() => {});
      ...
      // 受け入れ条件1: 片側失敗時に showError でエラー通知が表示される
      expect(mockShowError).toHaveBeenCalledWith(
        ERROR_MESSAGES.LIST.UPDATE_FAILED,
      );
      // 受け入れ条件2: 失敗時はクライアント状態を更新しない（成功時のみ状態反映）
      expect(mockUpdateListsAndTodos).not.toHaveBeenCalled();

      consoleSpy.mockRestore();
    });

    it('リスト更新成功後にTodo更新が失敗した場合、エラー通知が表示されクライアント状態は更新されない', async () => {
      const consoleSpy = vi
        .spyOn(console, 'error')
        .mockImplementation(() => {});
      ...
      // サーバー側の部分失敗（list更新済み・todo未更新）でも false を返す
      expect(editResult).toBe(false);
      expect(consoleSpy).toHaveBeenCalledWith(
        'Error puting list or todo:',
        expect.any(Error),
      ...
      // 受け入れ条件2: クライアント状態が部分更新で乖離しない（成功時のみ状態反映）
      expect(mockUpdateListsAndTodos).not.toHaveBeenCalled();
